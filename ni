...
incredible-pattern () { 
  # . ni; incredible-pattern nixosConfig ip4.address [--forcenoporecelain ][--remotecloneasroot] [--remoteinitbare] [--addpushurltoremote]
  ...
  git push; 
  # ssh root@"${2}" --  "cd ~/${PWD##*/}; pwd; git pull --set-upstream origin "/srv/local/git/$(git rev-list --parents HEAD | tail -1)_${PWD##*/}.git" && nixos-rebuild switch --flake .#${1}";   
